let Hands = ( sketch ) => {
  ...
  sketch.show = () => {
    ...
    for(let i = 0; i < hands_joints.length - 1; i++){
      for(var index in keypoints) {
        sketch.fill(0, 255, 0);
        let ratio = hands_joints[hands_joints.length - 1];
        let dx = (hands_joints[i][index][0]-sketch.width/2)*ratio[0] + ratio[2];
        let dy = (hands_joints[i][index][1]-sketch.height/2)*ratio[1] + ratio[3];
  ...
  sketch.drawLine = () => {
    ...
    junctions.forEach(pair => {
      ...
      for(let i = 0; i < hands_joints.length - 1; i++){
        let dx1 = (hands_joints[i][pair[0]][0]-sketch.width/2)*ratio[0] + ratio[2];
        let dy1 = (hands_joints[i][pair[0]][1]-sketch.height/2)*ratio[1] + ratio[3];
        ...
        let dx2 = (hands_joints[i][pair[1]][0]-sketch.width/2)*ratio[0] + ratio[2];
        let dy2 = (hands_joints[i][pair[1]][1]-sketch.height/2)*ratio[1] + ratio[3];
